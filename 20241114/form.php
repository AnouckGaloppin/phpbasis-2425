...
if (@$_POST['submit']) { // is "submit" als key aanwezig in de $_POST array? / staat submit op 'true'?
    //Bij het klikken op de submit-knop (deze heeft als name="submit")
    // wordt in de POST submit op true gezet. Je moet hier niets voor coderen, dit is altijd zo.
    // Hiermee controleer je of de submit-knop gewerkt heeft.
    $submitted = true;
    ...
    if (!isset($_POST['author'])) { // zit author in mijn POST?
        ...
        //Onderstaande is voor trolls-bescherming
    ...
    if (isset($_POST['status']) && (int)$_POST['status'] === 1) { // zit status in mijn POST én is deze gelijk aan 1 
        // (niet aangevinkte checkboxes worden niet via post meegestuurd)?  
        // niet aangevinkt wordt wel meegestuurd maar met status 0
        $status = 1;
    }
    ...
    if (count($errors) == 0) { // er werden geen fouten geregistreerd tijdens validatie
        ...
        header("Location: index.php?message=Record werd toegevoegd..."); // stuurt gebruiker terug naar hoofdpagina + melding in url
    }
}
            ...
            <form method="post" action="form.php">
                ...
                <div class="mb-3">
                    <select id="author" name="author" class="form-select" aria-label="Author">
                        ...
                        <!-- door de FETCH_KEY_PAIR (zie db.inc.php) is $authors een lijst van key-value pairs. Hierdoor moet je de index hebben 
                         (omdat die key is) om te verwijzen naar de authornaam (value)-->
                        <?php foreach ($authors as $index => $author): ?>
                            <!-- VERDER BEKIJKEN OM TE SNAPPEN -->
                ...
                <!-- type="submit" is altijd een knop, daarom is <button> hier overbodig -->
